...
import { CartOrdersModal } from './CartOrdersModal';
...
import { CaseType, UserCase, RouletteResult } from '../types/cases';
...
export default function CasesShopPageFixed({ 
  ...
  // Состояние для анимации бесплатного кейса
  const [freeRouletteModal, setFreeRouletteModal] = useState(false);
  const [isSpinning, setIsSpinning] = useState(false);
  const [hasSpun, setHasSpun] = useState(false);
  const [selectedFreeCase, setSelectedFreeCase] = useState<CaseType | null>(null);
  
  // Состояние корзины и заказов
  const [cart, setCart] = useState<CartItem[]>([]);
  const [orders, setOrders] = useState<Order[]>([]);
  const [cartOrdersModal, setCartOrdersModal] = useState(false);
  ...
  const handleFreeCaseOpen = () => {
    // Открываем модальное окно с рулеткой
    setFreeRouletteModal(true);
    setHasSpun(false);
  ...
  const handleRouletteResult = (result: RouletteResult) => {
    setSelectedFreeCase(result.selectedCase);
  };
  ...
  const handleCollectFreeCase = () => {
    if (selectedFreeCase) {
      const newCase: UserCase = {
        id: `user_case_${Date.now()}`,
        caseTypeId: selectedFreeCase.id,
        obtainedAt: new Date(),
        isOpened: false
      };
    ...
    setFreeRouletteModal(false);
    setIsSpinning(false);
    setHasSpun(false);
    setSelectedFreeCase(null);
    
    // Переключаемся на вкладку "МОИ КЕЙСЫ" чтобы показать полученный кейс
    ...
    <>
      ...
      >
        <Header 
          ...
          onCartClick={() => setCartOrdersModal(true)}
        />
                  ...
                  >
                    <div className="flex flex-col items-center text-center space-y-3">
                      ...
                      >
                        ...
                        />
                        <div 
                          className="absolute bottom-2 left-2 px-2 py-1 rounded unified-text"
                          style={{
                      ...
                      >
                        <div className="flex flex-col items-center text-center space-y-3">
                          ...
                          <button
                            className="w-full py-2 px-3 rounded-xl unified-button tracking-wide transition-all duration-200 transform hover:scale-105 hover:shadow-lg flex items-center justify-center text-center"
                            style={{
                              background: 'linear-gradient(145deg, #2B82FF, #1E62C2)',
                              color: '#FFFFFF',
                              boxShadow: '0 4px 15px rgba(43, 130, 255, 0.4)'
                            }}
                          >
                            ОТКРЫТЬ
                          </button>
      ...
      {/* Free Case Roulette Modal */}
      <ModalOpaque
        isOpen={freeRouletteModal}
        onClose={() => {
          if (!isSpinning) {
            setFreeRouletteModal(false);
            setIsSpinning(false);
        ...
        title={hasSpun && selectedFreeCase ? "Вы получили кейс!" : "Выбираем случайный кейс..."}
        theme={theme}
        actions={
          hasSpun && selectedFreeCase ? (
            <button
              onClick={handleCollectFreeCase}
              className="w-full transition-colors text-center"
              style={{
                height: '44px',
                borderRadius: '12px',
      ...
      >
        <div className="space-y-4">
          ...
          {/* Информация о выбранном кейсе */}
          {hasSpun && selectedFreeCase && (
            <div className="text-center space-y-3">
              <div 
                className="w-20 h-20 mx-auto rounded-xl overflow-hidden"
                style={{ 
                  background: `linear-gradient(145deg, ${selectedFreeCase.glowColor || selectedFreeCase.color}15, ${selectedFreeCase.glowColor || selectedFreeCase.color}05)`,
                  boxShadow: `0 0 30px ${selectedFreeCase.glowColor || selectedFreeCase.color}40, inset 0 0 20px ${selectedFreeCase.glowColor || selectedFreeCase.color}20`
                }}
              >
                <div className="w-full h-full flex items-center justify-center">
                  <ImageWithFallback 
                    src={selectedFreeCase.image}
                    alt={selectedFreeCase.name}
                    className="w-full h-full object-cover rounded-xl"
                    style={{ objectFit: 'cover' }}
                  />
                </div>
              ...
              <h4 
                ...
                style={{ 
                  color: theme === 'dark' ? '#FFFFFF' : '#0F172A', 
                  textShadow: theme === 'dark' 
                    ? `0 0 10px ${selectedFreeCase.color}80` 
                    : `0 0 5px ${selectedFreeCase.color}40`
                }}
              >
                {selectedFreeCase.name}
              </h4>
              ...
              <p 
                ...
                style={{ color: theme === 'dark' ? '#A7B0BD' : '#6B7280' }}
              >
                Кейс добавлен в вашу коллекцию!
              </p>
            </div>
          )}
          ...
          {isSpinning && (
            <div className="text-center">
              <p 
                className="unified-text animate-pulse"
                style={{ color: theme === 'dark' ? '#A7B0BD' : '#6B7280' }}
              >
                Выбираем кейс...
              </p>
      ...
      {/* Cart & Orders Modal */}
      <CartOrdersModal
        isOpen={cartOrdersModal}
        onClose={() => setCartOrdersModal(false)}
        cart={cart}
        orders={orders}
        shopItems={shopItems}
